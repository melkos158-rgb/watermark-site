...
def _save_upload(file_storage, subdir: str, allowed_ext: set) -> Optional[str]:
    ...
    - Якщо налаштовано Cloudinary → вантажимо туди.
    - Інакше пишемо локально в static/market_uploads/<subdir>/... і повертаємо /media/<subdir>/<name>.
    """
    ...
    rel_inside_market = os.path.join(subdir, unique_name).replace("\\", "/")
...
def api_upload():
    ...
    try:
        ...
        legacy_params = {
            ...
            "tags": tags_str,
            "cover": cover,
            "photos": json_dumps_safe({"images": images, "stl": stl_urls}),
            "file_url": file_url,
            "format": fmt,
            "user_id": user_id,
        }
