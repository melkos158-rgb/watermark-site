...
def create_app():
    ...
    def _inject_admin_metrics():
        try:
            ...
            if dialect_name == "sqlite":
                ...
                monthly_visitors = db.session.execute(
                    text(
                        """
                    SELECT COUNT(DISTINCT COALESCE(CAST(user_id AS TEXT), session_id))
                    FROM visits
    ...
    # ========= адмін-панель (листенер тільки тут, шаблон admin.html) =========
    ...
    def _track_visit():
        try:
            ...
            db.session.execute(
                text(
                    "INSERT INTO visits (session_id, user_id, path) "
                    "VALUES (:sid, :uid, :pth)"
                ),
    ...
    # ========= health-ендпоінти (залишаємо тут, щоб не плодити файлів) =========
